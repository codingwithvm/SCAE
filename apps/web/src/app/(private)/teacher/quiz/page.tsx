...
import {
  TEACHER_MCEES_QUESTIONS,
  TEACHER_MCEES_TOTAL,
  type TeacherAnswerValue,
} from "@/lib/teacher/quiz";

const STORAGE_KEY = "teacher_mcees_answers";
...
export default function TeacherQuizPage() {
  ...
  const question = TEACHER_MCEES_QUESTIONS[currentIndex];
  const currentAnswer = answers[question.id];
  const isFirst = currentIndex === 0;
  const isLast = currentIndex === TEACHER_MCEES_TOTAL - 1;
  const progressPercent = Math.round(
    (currentIndex / TEACHER_MCEES_TOTAL) * 100,
  );
      ...
      <header className="flex items-center justify-between h-16 px-8 bg-background border-b border-border-light shrink-0">
        ...
        <div className="flex flex-col items-center gap-0.5">
          ...
          <span className="text-xs text-text-secondary font-(family-name:--font-inter)]">
            Questão {currentIndex + 1} de {TEACHER_MCEES_TOTAL}
          </span>
        </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-primary font-(family-name:--font-inter)]">
                {currentIndex + 1}/{TEACHER_MCEES_TOTAL}
              </span>
            </div>
